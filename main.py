...
class App:
    ...
    async def pipeline(
        self, new_files_to_download: list = None, greenway_mode=False, mode="timelapse"
    ):
        self.greenway_mode = greenway_mode
        # FALSIFY GREENWAY_MODE TO RETURN TO NORMAL FUNCTIONALITY
        self.status = "Running pipeline..."
        logger.info("Starting pipeline. Downloading files...")
        ...
        if not self.greenway_mode:
            ...
            logger.info("Greenway Mode: Using local files without downloading.")
        ...
        self.processing_status = {
            file: {"stage": "Queued", "status": "Waiting to Start"}
        ...
        for file in files_to_process:
            ...
            self.processing_status["file"] = {
                "stage": "Processing",
                "status": f"Processing footage from {file}...",
            }
            logger.info(self.processing_status["file"]["status"])

            telemetry_objects = await self.frame_processor.process_video_pipeline(
                video_path=file, frame_rate=0.5, mode=mode
            )
            ...
            self.processing_status[file] = {
                "stage": "Complete",
                "status": f"Processing complete for {file}.",
            }
            logger.info(self.processing_status[file]["status"])
            ...
            if not greenway_mode:
                ai_events_created = await self.work_order_creator.ai_event_engine(
                    box_client=self.box, telemetry_objects=telemetry_objects
                )
                logger.info(f"AI Events created: {ai_events_created}")
    ...
    async def download_files(self, files_to_download: list = None) -> bool:
        for file in files_to_download:
            ...
            self.box.download_file(
                file_id=file["id"],
                file_name=file["name"],
                folder_path=self.box.unprocessed_videos_folder,
            )
    ...
    def clear_folders(self):
        ...
        for file in unprocessed_files:
            if file in self.processed_videos:
                shutil.move(
                    f"unprocessed_videos/{file}", f"{processed_videos_folder}/{file}"
                )
                logger.info(f"Moved {file} to {processed_videos_folder}")

    async def start_monitoring(
        self, interval=10, greenway_mode=False, mode="timelapse"
    ):
        ...
        try:
            while self.monitoring_active and five_min_timer > 0:
                ...
                if len(new_files_to_download) > 0:
                    ...
                    await self.pipeline(
                        new_files_to_download,
                        greenway_mode=self.greenway_mode,
                        mode=mode,
                    )
                else:
                    self.status = "Monitoring"
                    ...
                    if five_min_timer <= 0:
                        logger.info(
                            "Five-minute timeout reached with no new files. Shutting down monitoring."
                        )
                        self.monitoring_active = False
                        self.status = "Idle"
                        self.monitoring_status = "Idle"
